...
static void BM_ManualTiming(benchmark::State& state) {
  ...
  while (state.KeepRunning()) {
    ...
    // Spin to simulate a real workload while also accumulating CPU time
    auto target  = start + sleep_duration;
...
BENCHMARK(BM_ManualTiming)->Range(1, 1 << 14)->UseRealTime()->UseManualTime();
